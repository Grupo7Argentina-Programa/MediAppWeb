...
public class EditarPromocionServlet extends HttpServlet implements Servlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/edit-promocion.jsp");
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		Promocion promocion = promocionService.update(id, tipoDePromocion, nombre, costo, descuento, atraccion1, atraccion2,
				atraccion3, atraccion4);
		if (promocion.isValid()) {
			req.setAttribute("flash", "La promoción fue actualizada con éxito");
			resp.sendRedirect("listado-promociones.do");
		} else {
			req.setAttribute("promocion", promocion);

			RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/edit-promocion.do");
			dispatcher.forward(req, resp);
		}
